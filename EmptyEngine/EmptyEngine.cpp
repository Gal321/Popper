#include <SDL_syswm.h>
#include "EE_MainClass.h"

void EE::Engine::EUpdate(int Ticks)
{
	SDL_Event event;

	(Ticks);

	while (SDL_PollEvent(&event))
	{
		switch (event.type)
		{
		case SDL_QUIT:
			this->EngineState = true;
			break;

		case SDL_KEYDOWN:
			this->input.KeyDown(static_cast<EE::Byte>(event.key.keysym.sym));
			break;

		case SDL_KEYUP:
			this->input.KeyUp(static_cast<EE::Byte>(event.key.keysym.sym));
			break;

		case SDL_SYSWMEVENT:
			switch (event.syswm.msg->subsystem)
			{
			case SDL_SYSWM_WINDOWS:
				if (event.syswm.msg->msg.win.msg == 0x312 /* WM_HOTKEY */)
				{
					this->isHotkeyPressed = true;
				}
				break;

			default:
				break;
			}
			break;
		}
	}
	/*for (auto obj : EE::GraphicEntity::listOfUIEnt)
		obj->Step(Ticks);*/
}

EE::Engine::Engine(std::wstring title):
	EngineState(0),
	window(nullptr),
	renderer(nullptr),
	isHotkeyPressed(false)
{
}

EE::Engine::~Engine()
{
	if (!this->window)
		SDL_DestroyWindow(this->window);

	SDL_Quit();
}

bool EE::Engine::Initialize()
{
	SDL_SysWMinfo info = { 0 };

	if (SDL_Init(SDL_INIT_VIDEO) < 0)
	{
		printf("SDL could not initialize! SDL_Error: %s\n", SDL_GetError());
		return false;
	}

	if (TTF_Init() == -1)
	{
		printf("SDL_ttf could not initialize! SDL_Error: %s\n", SDL_GetError());
		return false;
	}

	//Create window
	this->window = SDL_CreateWindow(this->windowTitle.c_str(),
									SDL_WINDOWPOS_CENTERED,
									150,
									DEFAULT_WIDTH,
									DEFAULT_HEIGHT,
									SDL_WINDOW_HIDDEN | SDL_WINDOW_BORDERLESS);
	if (this->window == NULL)
	{
		printf("Window could not be created! SDL_Error: %s\n", SDL_GetError());
		return false;
	}

	//Get window surface
	this->renderer = SDL_CreateRenderer(this->window,
										-1,
										SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
	if (this->renderer == NULL)
	{
		printf("Renderer could not be created! SDL_Error: %s\n", SDL_GetError());
		return false;
	}

	//Enable the custom processing of SDL window manager events
	SDL_EventState(SDL_SYSWMEVENT, SDL_ENABLE);

	//Sets the hotkey to show Popper
	if (SDL_GetWindowWMInfo(this->window, &info) == SDL_FALSE)
	{
		printf("Couldn't get the system info! SDL_Error: %s\n", SDL_GetError());
		return false;
	}
	if (RegisterHotKey(info.info.win.window, 1, MOD_CONTROL | MOD_ALT, 'O') == FALSE)
	{
		printf("Couldn't set the wanted hotkey! WIN32_Error: %s\n", GetLastError());
		return false;
	}

	return true;
}

void EE::Engine::Uninitialize()
{
	/*for (auto obj : EE::BaseGraphic::listOfGO)
		obj->Uninitialize();*/
	SDL_DestroyRenderer(this->renderer);
}

bool EE::Engine::ERender()
{
	SDL_RenderClear(this->renderer);

	/*for (auto obj : EE::BaseGraphic::listOfGO)
		if (obj->toRender)
			obj->DrawObject();*/

	SDL_RenderPresent(this->renderer);
	
	return true;
}
void EE::Engine::GetSize()
{
	/*RECT rc;
	GetClientRect(m_hwnd, &rc);
	if (rc.right*(9.0f / 16.0f) < rc.bottom)
		return D2D1::SizeF(static_cast<float>(rc.right), static_cast<float>(rc.right*(9.0f / 16.0f)));
	else return D2D1::SizeF(static_cast<float>(rc.bottom*(16.0f / 9.0f)), static_cast<float>(rc.bottom));*/
}

bool EE::Engine::CheckHotkey()
{
	bool temp = this->isHotkeyPressed;
	this->isHotkeyPressed = false;
	return temp;
}

void EE::Engine::ShowWindow()
{
	SDL_ShowWindow(this->window);
}

void EE::Engine::HideWindow()
{
	// TODO - CLEAR THE DATA

	SDL_HideWindow(this->window);
}